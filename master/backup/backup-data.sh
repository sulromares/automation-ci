...
curl -s -H "Authorization: Bearer ${token}" -H "Content-Type: application/x-gzip" --data-binary @jenkins_data_${now}.tar.gz "https://www.googleapis.com/upload/storage/v1/b/${bucket}/o?uploadType=media&name=jenkins_data_${now}.tar.gz"
...
echo "Uploading to GCS..."
curl -s -H "Authorization: Bearer ${token}" -H "Content-Type: application/x-gzip" --data-binary @nexus_data_${now}.tar.gz "https://www.googleapis.com/upload/storage/v1/b/${bucket}/o?uploadType=media&name=nexus_data_${now}.tar.gz"
...
echo "Uploading to GCS..."
curl -s -H "Authorization: Bearer ${token}" -H "Content-Type: application/x-gzip" --data-binary @sonar_data_${now}.tar.gz "https://www.googleapis.com/upload/storage/v1/b/${bucket}/o?uploadType=media&name=sonar_data_${now}.tar.gz"
...
echo "Uploading to GCS..."
curl -s -H "Authorization: Bearer ${token}" -H "Content-Type: application/x-gzip" --data-binary @postgres_data_${now}.tar.gz "https://www.googleapis.com/upload/storage/v1/b/${bucket}/o?uploadType=media&name=postgres_data_${now}.tar.gz"
